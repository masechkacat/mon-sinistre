...
Живой номер в закрывающем трейлере — ровно один: тот, что относится к этому коммиту. GitHub ищет ключевые слова (`Closes`, `Fixes`, `Resolves` и формы) по всему тексту сообщения коммита, заголовка и тела PR и оформление не разбирает — строка «Closes: #12», написанная в кавычках как пример работы Ralph Loop, закрыла issue #12 при мерже PR #19 (02.08.2026, коммит `f7075e6`). Задача не была сделана и выпала из плана фазы: цикл считает закрытый issue сделанным. Примеры и объяснения трейлера писать только с плейсхолдером — `Closes #<N>`; на бэктики не рассчитывать.
