...
## Ветка `subscribe` реализует пока две трети сценария

Ветвление, гонка `P2002` и всё остальное — в docblock'ах
`VeilleService.upsertSubscription` и `resubscribeUnconfirmed`, здесь не
пересказывается. Что оттуда не видно: третья ветка research (подтверждённый
адрес получает письмо «déjà inscrit·e») всё ещё не реализована — пока этот
случай молча ничего не делает, как и заменённое им временное поведение с
проглоченным `P2002`.
