...
- `dto/veille-confirmation-token.dto.ts` — одна DTO с полем `token`,
  общая для `GET` (`@Query`) и `POST` (`@Body`) confirmation: Nest валидирует
  класс одинаково независимо от декоратора-источника, второй такой же DTO не
  заводить.
...
- `veille-token.ts` — единственный способ получить пару токен/хеш
  (`generateVeilleToken`) и пересчитать хеш по токену
  (`hashVeilleToken`, использует статус подтверждения; отписка фазы 2 —
  следующая задача): `randomBytes(32).base64url` в письмо, `sha256` hex в
  базу, второй генерации не заводить.
