...
регистрацию неподтверждённым адресом (фаза 3, начата); повторная регистрация
подтверждённым адресом — та же фаза, ещё не реализована; документируется
...
- `POST /auth/register` → `AuthService.register`; всегда `204`, каким бы ни
  ...
  оставлен как есть, письмо «у вас уже есть аккаунт» —
  `docs/plan/user-account.md`, фаза 3, следующий issue, пока не реализовано.
  Механика каждой ветки — в докблоках `register`, `claimUnconfirmedAccount` и
  `resendConfirmationMail`, здесь не пересказывается.
...
- `register`: ветка нового адреса и ветка неподтверждённого адреса обе ждут
  `mail.send()` — как у трёх веток `upsertSubscription` в veille. Осталась
  одна асимметрия: ветка уже подтверждённого адреса отвечает сразу, без
  письма — «у вас уже есть аккаунт» ещё не реализовано (см. выше), это и
  выровняет последнюю ветку.
