...
describe('account mail rate limit (ACCOUNT_EMAIL_LIMIT)', () => {
  ...
  it('suppresses the sixth account mail within 24h; the form still answers normally', async () => {
    const email = 'victime@example.fr';

    for (let i = 0; i < ACCOUNT_EMAIL_LIMIT; i++) {
      const res = await register(email);
      expect(res.statusCode).toBe(204);
    }
    expect(transport.sent).toHaveLength(ACCOUNT_EMAIL_LIMIT);

    const sixth = await register(email);
  ...
  it('counts every account mail kind toward the same limit, not one per mail', async () => {
    const email = 'victime@example.fr';

    for (let i = 0; i < ACCOUNT_EMAIL_LIMIT - 1; i++) {
      await register(email);
    }
    expect(transport.sent).toHaveLength(ACCOUNT_EMAIL_LIMIT - 1);

    // The confirmation mails above already spent 4 of the 5; a password-reset
    // mail to the same address spends the last one.
    await requestPasswordReset(email);
    expect(transport.sent).toHaveLength(ACCOUNT_EMAIL_LIMIT);

    const blocked = await register(email);
