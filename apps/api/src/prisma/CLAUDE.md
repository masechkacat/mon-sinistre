# CLAUDE.md — `src/prisma`

Подключение Prisma 7, сборка connection string, экранирование LIKE и клиент для
интеграционных тестов. Схема и миграции живут в `apps/api/prisma/`.

**Стиль v7 — driver adapter**: инструкции из туториалов для Prisma 5/6 не
годятся.

## Конфигурация CLI

**`prisma.config.ts`** в корне пакета — единственный источник конфигурации CLI.
Первой строкой
`process.loadEnvFile()` в `try/catch`: CLI в v7 сама `.env` **не** загружает, а
при отсутствии файла (например в CI) переменные приходят из окружения. `dotenv`
не подключать — фантомная зависимость, Node ≥ 24 умеет это сам. Здесь же
настроена seed-команда: `migrations.seed` →
`ts-node -r tsconfig-paths/register prisma/seed.ts` (регистрация путей
обязательна: value-импорт из `src/...` без неё упадёт с `MODULE_NOT_FOUND`).

## `DATABASE_URL` не существует

Connection string собирает `database-url.ts` из
`DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME`. Так docker-compose, CLI
миграций, рантайм и интеграционные тесты гарантированно смотрят на одну базу.

Значения приходят двумя путями, и пути не равноправны: внутри Nest — только из
`ConfigService`, снаружи (конфигурация CLI, seed, клиент интеграционных тестов)
— через `database-url-from-env.ts`. В коде приложения эту функцию не вызывать:
она читает `process.env` и тем самым обходит проверку схемы окружения.

## Схема и клиент

**`schema.prisma`**: url в `datasource db` не пишется — он живёт в конфиге.
Генератор эмитит TypeScript в `src/generated/prisma` (в `.gitignore`);
`npm run prisma:generate` — после каждой правки схемы, `npm run build` делает
это сам.

**`PrismaService`**: явный `$connect` в `onModuleInit` — недоступная база валит
приложение при bootstrap, как и невалидный env. API без Postgres не стартует,
это осознанно.

## `escapeLikePattern`

`\`, `%`, `_` → с обратной косой одним проходом по классу символов (Postgres
читает обратную косую как escape по умолчанию). Это **единственное правило
экранирования на весь API**: применять к любому пользовательскому вводу в
`startsWith`/`contains`/`endsWith`, второй реализации не заводить. Ровно один
раз — функция не идемпотентна, повторный вызов экранирует собственные escape'ы.

Запросы остаются типизированным Prisma — `$queryRaw` ради поиска не заводить.

## Перевод ошибок запроса

`prisma-error.ts`; правило и запрет на `P2002` — `../../CLAUDE.md`, границы
того, что можно сказать наружу, — в комментариях самого файла. Там же
`isUniqueViolationOn` — единственный способ спросить «`P2002` по такому-то
полю?»: за driver adapter нарушенные колонки лежат не в `meta.target`, и
самодельная проверка по туториалу для v5/v6 молча отвечает «нет». Там же
`isForeignKeyViolation` — единственный способ спросить «упал `P2003`?», для
`create`, чей родитель мог исчезнуть между чтением, выбравшим эту ветку, и
самой записью. Знание про Prisma живёт здесь, а не в фильтре исключений:
фильтр генерируемый клиент не импортирует.

## Клиент для интеграционных тестов

`test/helpers/prisma-client.ts` — `createIntTestPrismaClient` для спеков, которым
Nest не нужен (импорт работает вне контекста, тесты миграций — на сыром SQL).
`$disconnect` в `afterAll` — на вызывающей стороне, иначе прогон повиснет на
открытом пуле. Спеки, поднимающие `AppModule`, берут `PrismaService` из
контейнера, а не этот хелпер.
