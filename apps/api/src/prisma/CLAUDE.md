...
`prisma-error.ts` — что упавший запрос значит снаружи. В таблице одна запись:
`P2025` → 404 (чужой объект приходит сюда как «не найдено» и обязан уйти как
404). Код попадает в неё, только если один ответ верен сразу для всех
эндпоинтов; почему `P2002` не тот случай и чем это грозит — в комментарии над
таблицей и в `docs/decisions.md` (03.08.2026). Знание про Prisma живёт здесь, а
не в фильтре исключений, — он превращает исключения в ответы и генерируемый
клиент не импортирует.
