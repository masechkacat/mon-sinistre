...
Колонка `nameNormalized` заведена миграцией `add_commune_name_normalized`: **nullable**, `COLLATE "C"`, обычный btree `Commune_nameNormalized_idx`. Байтовый порядок здесь не оптимизация, а требование переносимости: нормализация снимает регистр и диакритику, но не пунктуацию, а расходятся collation'ы именно на ней — glibc игнорирует дефис на первичном уровне, musl и ICU нет, и «Saint-Étienne» с «Sainte-Marie» меняются местами между `postgres:18` и `postgres:18-alpine` (проверено 02.08.2026 на обоих образах). Под `COLLATE "C"` обычный btree обслуживает и префиксный `LIKE`, и `ORDER BY`, поэтому класс оператора `text_pattern_ops` не нужен (под локальной collation понадобился бы). Collation Prisma не поддерживает и в схеме не выражает — она живёт только в SQL миграции и дрейфом не считается; сторожит её int-тест, потому что `migrate dev` потерю collation молча пропустит.
...
- **Логи — только через `Logger` из `@nestjs/common`**, с контекстом класса (`private readonly logger = new Logger(CommuneImportService.name)`); `console.log`/`console.error` в коде приложения не используются (сообщение без контекста и уровня теряется в проде). Уровни задаются один раз в `main.ts` при создании приложения: `error`/`warn`/`log` в продакшене, плюс `debug`/`verbose` в остальных средах. Ошибка логируется с уровнем `error` и стеком, а не `log`. Структурированный JSON-транспорт (`nestjs-pino`) — отдельное решение на момент, когда появится сбор логов; до тех пор зависимость не заводить.
...
- **Swagger-схема свойств — через CLI-плагин `@nestjs/swagger`** (`plugins` в `nest-cli.json`, `introspectComments: true`): типы полей и JSDoc-комментарии DTO попадают в `/docs` без ручных декораторов. `@ApiProperty` пишется только там, где плагину нечего вывести: пример значения, описание вне JSDoc, брендированные типы contracts (`IsoDate` стирается компилятором до `string` — указывать `type: String, format: 'date'`). Response-класс с `implements` контракта нужен по-прежнему: интерфейсы `@mon-sinistre/contracts` на рантайме отсутствуют, плагин их не видит. Ограничение: плагин — AST-трансформер `nest build`, ts-jest его не применяет, поэтому под тестами схема беднее боевой — проверять её на запущенном приложении (`/docs`), а не в тестах.
