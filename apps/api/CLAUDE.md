...
Нормализация названий (`src/communes/normalize-commune-name.ts`): `normalizeCommuneName` — единственный способ получить поисковый ключ названия коммуны (lowercase → NFD → удаление `\p{M}` → `œ`→`oe`, `æ`→`ae` → `’`→`'`). Ею предстоит заполнять колонку `Commune.nameNormalized` при импорте и нормализовать `q` в запросе (импорт и поиск переводятся на неё дальше в фазе 3); второй реализации правила быть не должно, иначе запись и чтение разъедутся. Апострофы, дефисы и пробелы сохраняются осознанно как разделители («lhay» не находит «L'Haÿ-les-Roses»), но их типографские варианты приводятся к одному — `docs/decisions.md` (02.08.2026). Функция даёт ключ только для названий: ветка поиска по коду INSEE сравнивает «сырой» `q` с `codeInsee`, который импорт хранит как есть, в верхнем регистре (`2A004`). Тест — `normalize-commune-name.spec.ts`, без базы.

Колонка `nameNormalized` заведена миграцией `add_commune_name_normalized`: **nullable** плюс индекс `Commune_nameNormalized_prefix_idx` — btree с `text_pattern_ops` (обычный btree под не-C collation префиксный `LIKE` не ускоряет). Индекс объявлен и в `schema.prisma` (`ops: raw(...)`), и в SQL миграции: без объявления в схеме следующий `migrate dev` счёл бы его дрейфом и снёс. Обратно из базы класс оператора Prisma не читает, поэтому каждая новая миграция будет предлагать эквивалентный `DROP INDEX` + `CREATE INDEX` этого индекса — такой блок из сгенерированного файла удалять. Правило нормализации не повторяется в SQL: backfill существующих строк — перезапуск идемпотентного импорта (`npm run seed`) после того, как импорт научится заполнять колонку. Ужесточение до `NOT NULL` — отдельной миграцией, и только после этого backfill'а: раньше она упала бы на любой засеянной базе и сломала бы импорт, который колонку ещё не пишет. Тест на структуру (колонка nullable + индекс) — `commune-name-normalized.int-spec.ts`.

Модули: `CommunesModule` — публичный поиск `GET /communes?q=` (префикс названия или точный код INSEE, только действующие коды, лимит `COMMUNE_SEARCH_LIMIT` из contracts; до фазы 3 — по «сырому» `name`, нормализация регистра и диакритики придёт с `nameNormalized`). Принятое до фазы 3 ограничение: `startsWith` Prisma не экранирует LIKE-символы `%` и `_` — при реализации поиска по `nameNormalized` ввод экранировать (комментарий в `communes.service.ts`). Индекс под префиксный поиск уже заведён миграцией (см. абзац о `nameNormalized` выше). Swagger-документация эндпоинтов — вручную (`@ApiProperty` в DTO, `@ApiOkResponse` с response-классом, реализующим контракт через `implements`): CLI-плагин `@nestjs/swagger` не подключён.
