...
- **Строки, которые читает пользователь продукта (письма, тексты ответов), — только в `src/i18n/fr.ts`**: типизированный словарь `as const`, импорт напрямую, параметры — функциями-полями; фичи добавляют свои строки в ветки того же объекта, второго файла и текстов в `packages/contracts` быть не должно. Административные термины употребляются только в составе объясняющей формулировки (`fr.mail.terms.arreteCatNat`) — сторож `src/i18n/fr.spec.ts` проверяет это, а заодно отсутствие разметки и типографику по всему словарю. Почему словарь, а не i18n-библиотека — `docs/research/emails.md`. Описание OpenAPI в `main.ts` под правило не подпадает: его читают разработчики в `/docs`, а не пользователи.
- **Письма.** Модуля отправки ещё нет — он появляется в фазе 1 фичи `emails` (план — `docs/plan/emails.md`). Технические решения приняты заранее и обязательны для всех фаз: провайдер Scaleway TEM (регион `fr-par`, HTTP API через нативный `fetch`), французские строки — по правилу выше, text- и HTML-версии рендерятся из одной блочной модели, локальный режим пишет письма в файлы. Основания и ограничения — `docs/research/emails.md`.
