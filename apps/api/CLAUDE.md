...
Чтение по имени, которого в схеме нет, не компилируется. `getOrThrow` остаётся
...
- Глобальный `ValidationPipe` с `whitelist + forbidNonWhitelisted`: у каждого
  ...
  отклонено. Пайп создаёт `createGlobalValidationPipe` (`src/config/`) — её же
  использовать в интеграционных тестах, конфигурацию не копировать.
...
- **Необработанные ошибки — через `AllExceptionsFilter`** (`src/common/`,
  ...
  обработчиках не заводить. Перевод кодов Prisma — `src/prisma/prisma-error.ts`:
