...
Колонка `nameNormalized` заведена миграцией `add_commune_name_normalized`: **nullable** плюс индекс `Commune_nameNormalized_prefix_idx` — btree с `text_pattern_ops` (обычный btree под не-C collation префиксный `LIKE` не ускоряет). Индекс объявлен и в `schema.prisma` (`ops: raw(...)`), и в SQL миграции: без объявления в схеме следующий `migrate dev` счёл бы его дрейфом и снёс. Обратно из базы класс оператора Prisma не читает, поэтому каждая новая миграция будет предлагать эквивалентный `DROP INDEX` + `CREATE INDEX` этого индекса — такой блок из сгенерированного файла удалять. Правило нормализации не повторяется в SQL: backfill существующих строк — перезапуск идемпотентного импорта (`npm run seed`), который колонку теперь заполняет. Ужесточение до `NOT NULL` — отдельной миграцией и только после этого backfill'а: на засеянной до фазы 3 базе строки без ключа остались, и миграция упала бы на них. Тест на структуру (колонка nullable + индекс) — `commune-name-normalized.int-spec.ts`.

Модули: `CommunesModule` — публичный поиск `GET /communes?q=` (префикс названия или точный код INSEE, только действующие коды, лимит `COMMUNE_SEARCH_LIMIT` из contracts). Поиск по названию идёт по `nameNormalized`: `q` проходит через `normalizeCommuneName`, поэтому «chateau» находит «Château-Thierry»; сортировка — по той же колонке, а не по `name`, и потому не зависит от collation базы (под byte-order collation «Étampes» уехал бы в конец выдачи). Ветка кода INSEE сравнивает «сырой» `q`. `startsWith` Prisma не экранирует LIKE-символы, поэтому нормализованный префикс проходит через `escapeLikePattern` (`\`, `%`, `_` → с обратной косой; Postgres читает её как escape по умолчанию) — так запрос остаётся типизированным Prisma, без `$queryRaw`; ввод в LIKE, добавленный где-то ещё, экранировать так же. Индекс под префиксный поиск заведён миграцией (см. абзац о `nameNormalized` выше). Swagger-документация эндпоинтов — вручную (`@ApiProperty` в DTO, `@ApiOkResponse` с response-классом, реализующим контракт через `implements`): CLI-плагин `@nestjs/swagger` не подключён.
