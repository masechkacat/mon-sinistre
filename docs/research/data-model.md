...
`docs/decisions.md` (записи от 30.07.2026: версионирование `DeadlineRule`,
справочник коммун COG, EXIF-ограничение).
...
| id                   | `uuid` (v7 — сортируемость по времени), генерация на стороне БД                                                                               |
...
erDiagram
    ...
    StepTemplate }o--o{ Step : copied-from
...
Решение от 30.07.2026 (`decisions.md`): effective-dated, резолв по дате якоря.
...
выражает — дописать SQL в файл миграции руками (миграции это позволяют).
...
подписка молча умирает при слиянии) и подпись «теперь часть X». Строки Veille и
...
- `File`: id, inventoryItemId → InventoryItem (cascade), kind (PHOTO /
  ...
  загрузки, задел на дедупликацию), uploadedAt. EXIF-ограничение из `decisions.md` — к пайплайну загрузки, не к
  схеме.
...
- `ReminderLog` (напоминания шагов): stepId → Step (cascade), **kind**
  ...
  (факт: за сколько дней отправлено; для `OVERDUE` не используется), sentOn
  (date); `unique(stepId, kind, offsetDays)` для `SCALE`,
  ...
  число-факт, а не код причины: при смене шкалы история остаётся читаемой.
...
1. ~~**Commune**: достаточно ли схемы «код + преемник» без истории
   ...
   деградирует в штатный алерт. Формулировка в `decisions.md` уточнена.
